...
- Store detail invoke docs are generated by `src/lib/invoke-parameter-docs.ts`; when changing `/api/store/invoke` request contract (fields/enums/GET-vs-POST behavior), update that helper and its tests together.
- `UnifiedAgent.inputSchema` is the structured invocation contract (`type: "object"`, `properties`, `required`); provider adapters should at least emit `createOpenInputSchema(method)` so detail docs remain schema-driven even without provider-declared fields.
