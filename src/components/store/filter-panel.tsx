...
const MOCK_CATEGORIES = ["DeFi", "NFT", "Analytics", "Trading", "Infrastructure", "Social"]
...
export function FilterPanel({ className }: { className?: string }) {
    ...
    return (
        <aside className={cn("w-full md:w-64 shrink-0 flex flex-col gap-6", className)}>
                    ...
                    {["all", "online", "offline"].map((s) => (
                        <div key={s} className="flex items-center space-x-2">
                            <Checkbox
                                id={`status-${s}`}
                                checked={status === s}
                                onCheckedChange={() => setStatus(s as "all" | "online" | "offline")}
                            />
                            <Label
                                htmlFor={`status-${s}`}
